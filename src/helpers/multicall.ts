...
import { MULTICALL_ADDRESS, RPC_URL } from "src/constants/const";
...
export const multicall = async <T = any>(abi: any[], calls: Call[],  provider?:StaticJsonRpcProvider | JsonRpcProvider | BaseProvider, options?: MulticallOptions): Promise<T> => {
    const { requireSuccess = true, ...overrides } = options || {}
    
    const multi = MulticallContract__factory.connect(MULTICALL_ADDRESS, provider??simpleRpcProvider)
